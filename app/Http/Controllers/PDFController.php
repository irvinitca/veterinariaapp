...
class PDFController extends Controller
{
    ...
    public function generatePDFIngresosVet(Request $request)
    {
        $date_start = $request->input('date_start');
        $date_end = $request->input('date_end');
        $user_id = $request->input('user_id');
        ...
        $data = [
            'title' => 'Reporte de Ingresos - Dr./Dra.:  '.$user->name,
            'desde' => Carbon::parse($date_start)->format('d M y'),
            'hasta' => Carbon::parse($date_end)->format('d M y'),
            'name' => $user->name,
            'total_ingresos' => $appointments->sum('total'),
            'appointments' =>$appointments
        ];

        $pdf = PDF::loadView('admin.reporteIngresosVet', $data);

        return $pdf->download('reporteIngresosPorVet.pdf');
    }

    public function pdfIngresosVet(){
        ...
        return view('admin.formReporteIngresosVet' , compact('users'));
    }
